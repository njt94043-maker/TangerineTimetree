...
- [ ] **S36: Web audio engine + Library redesign** — TypeScript Web Audio API click scheduling (frame-accurate). Track playback (fetch + AudioContext.decodeAudioData). Stem mixing (per-channel GainNode). SoundTouchJS pitch-preserved speed control. A-B loop region. Web Library redesign (Songs/Setlists tabs, category/type filter pills, inline Live/Practice launch). Player prefs loading from user_settings.
- [ ] **S37: Web player UI** — React player component (visual hero, transport controls, queue overlay, set complete screen, between-songs waiting screen). Live mode (click + visuals + lyrics/chords, stage prompter merged). Practice mode (full mixer + tracks + stems + speed slider + A-B loop UI). Per-user prefs toggles in settings. Wake lock API. Waveform visualiser (Canvas). Beat glow animation (D-119 card-level).
