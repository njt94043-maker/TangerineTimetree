...
- [ ] **S33: Songs/Setlists/Live/Practice big-picture redesign** — Planning session needed. Song categories (Tange covers/originals, personal songs per member). Setlist types (Tange setlists, other band setlists for standing-in). Live mode = click+visuals only (3 playback modes: full library, filtered, setlist). Practice mode = backing tracks + click+visuals (same 3 modes). Speed safety check before live mode. Schema changes, mockups, full flow design. Approved mockup: `mockups/practice-redesign.html` (v5). C++ stop() fixed for loop-aware reset.
