...
#endif
// samplerate.h removed: we use hopSize=512 exclusively so onsetDFBufferSize=512
// and resampleOnsetDetectionFunction() becomes a 1:1 copy (ratio=1.0)
...
void BTrack::initialise (int hop)
{
    ...
    sampleRate_ = 44100; // default; call setSampleRate() to override
    tightness = 5;
    alpha = 0.9;
...
{	
	...
	beatPeriod = round (60 / ((((double) hopSize) / sampleRate_) * 120.));
...
void BTrack::setTempo (double tempo)
{
	...
	int newBeatPeriod = (int) round (60 / ((((double) hopSize) / sampleRate_) * tempo));
...
//=======================================================================
void BTrack::setSampleRate (int sr)
{
    sampleRate_ = (sr > 0) ? sr : 44100;
    // Recompute beat period with the new sample rate
    setHopSize (hopSize);
}
...
{
    // With hopSize=512: onsetDFBufferSize = (512*512)/512 = 512
    // ratio = 512/512 = 1.0  →  direct copy, no resampling required
    for (int i = 0; i < 512; i++)
...
{
    double tempoToLagFactor = 60. * (double)sampleRate_ / (double)hopSize;
	...
	beatPeriod = round ((60.0 * (double)sampleRate_) / (((2 * maxIndex) + 80) * ((double) hopSize)));

	if (beatPeriod > 0)
        estimatedTempo = 60.0 / ((((double) hopSize) / (double)sampleRate_) * beatPeriod);
}
