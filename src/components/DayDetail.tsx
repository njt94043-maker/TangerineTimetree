    ...
    <div className="overlay">
      ...
      <div className="day-sheet neu-card">
        ...
        {!loading && gigs.length === 0 && (
          <p style={{ color: 'var(--color-text-dim)', textAlign: 'center', padding: '20px 0' }}>No gig booked</p>
        )}

        {gigs.map(gig => (
          <div key={gig.id} className="neu-inset" style={{ padding: 14, marginBottom: 12, cursor: 'pointer' }} onClick={() => onEditGig(gig.id)}>
            {isGigIncomplete(gig) && <span className="incomplete-badge">INCOMPLETE</span>}

            <div style={{ fontSize: 16, fontWeight: 700 }}>{gig.venue || 'Venue TBC'}</div>
            <div style={{ fontSize: 13, color: 'var(--color-text-dim)', marginBottom: 10 }}>{gig.client_name || 'Client TBC'}</div>

            <div className="detail-grid">
              <div className="detail-row"><span className="detail-label">Fee</span><span className="detail-value">{fmtFee(gig.fee)}</span></div>
              <div className="detail-row"><span className="detail-label">Payment</span><span className="detail-value">{gig.payment_type || '\u2014'}</span></div>
              <div className="detail-row"><span className="detail-label">Load-in</span><span className="detail-value">{fmt(gig.load_time)}</span></div>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            {awayOnDate.map(a => (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', padding: '4px 0' }}>
                <span style={{ fontWeight: 700, fontSize: 13, color: 'var(--color-partial)' }}>{a.user_name}</span>
        ...
        <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button className="btn btn-tangerine" onClick={() => onAddGig(date)}>Add Gig</button>
